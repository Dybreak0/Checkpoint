...
{
    class NewJOSecondViewModel : BaseViewModel
    {
        ...
        public NewJOSecondViewModel(IMvxNavigationService navigationService, 
        ...
        public IMvxAsyncCommand GoToCaseTaggingPageCommand => new MvxAsyncCommand(async () =>
        {
            ...
            if (IsValidFields(secondPageVM))
            {
                ...
                if (!_parameter.ContainsKey(Constants.Params.FromSecondPage)) { _parameter.Add(Constants.Params.FromSecondPage, string.Empty); }

                if(_parameter.ContainsKey(Constants.Params.EditedCasesSelected)) { _parameter.Remove(Constants.Params.EditedCasesSelected);  }

                if (_parameter.ContainsKey(Constants.Params.IsEdit)) { _parameter.Remove(Constants.Params.IsEdit); }

                await _navigationService.Navigate<CaseTaggingViewModel, Dictionary<string, string>>(_parameter);
            }            
        });
