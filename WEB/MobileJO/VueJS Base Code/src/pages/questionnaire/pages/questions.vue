    ...
    <v-layout column>
        ...
        <v-form v-model="valid" ref="form">
            <v-layout row v-if="!disabled && loaded" class="question-flex">
                ...
                <v-flex md4 xs4>
                    <v-btn @click="saveQuestion" class="btn_primary">
                        <v-icon>add</v-icon>
                        Add
                    </v-btn>
                </v-flex>
                ...
                <v-card class="question-card">
                        ...
                        <v-layout row>
                            ...
                            <v-flex xs6 md6 class="actions-flex">
                                <v-layout
                                    row
                                    class="actions-row"
                                    v-if="activeQuestion != item.question_id"
                                >
                                    <v-btn
                                        @click="
                                        ...
                                        :class="disabled ||
                                            (activeQuestion !=
                                                item.question_id &&
                                                0 != activeQuestion)
                                                ? 'mx-2 actions-question btn_primary disable-events'
                                                : 'mx-2 actions-question btn_primary'
                                        "
                                        fab
                                    >
                                        <v-icon class="actions-icon"
                                            >delete</v-icon
                                        >
                                    </v-btn>
                                    <v-btn
                                        @click="editQuestion(item.question_id)"
                                        :class="
                                            disabled ||
                                            (activeQuestion !=
                                                item.question_id &&
                                                0 != activeQuestion)
                                                ? 'mx-2 actions-question disable-events'
                                                : 'mx-2 actions-question'
                                        "
                                        fab
                                        small
                                        color="#1332A8"
                    ...
                    <v-container class="question-body">
                        ...
                        <v-flex
                            class="add-flex"
                            v-if="
                                2 == item.question_type_id ||
                                    3 == item.question_type_id
                            "
                        >
                            <v-btn
                                @click="addChoice(item.question_id, index)"
                                :class="
                                    disabled || null != activeChoice
                                        ? 'mx-2 actions-question disable-events'
                                        : 'mx-2 actions-question'
                                "
                                fab
                        ...
                        <v-flex
                            class="choice-div"
                            v-for="(data, i) in item.Choices"
                            v-if="
                                2 == item.question_type_id ||
                                    3 == item.question_type_id ||
                                    7 == item.question_type_id
                            "
                            :key="data.choice_id"
                        >
                            <choicesTemplate
                                ...
                                @resetOld="resetOld"
                                @retrieveTemplate="retrieve"
                                :disabled="isDisabledF(data.choice_id)"
                                :isActive="isActive(i, item.question_id)"
                                @noActiveChoice="noActiveChoice"
...
export default {
    data: () => ({
        ...
        questionRules: [v => !!v || constants.fillRequireFieldsError],
        status: true,
        activeQuestion: 0,
        oldQuestion: "",
        currentQuestion: [],
    ...
    methods: {
        ...
        isActive(id, question_id) {
            return (
                id == this.activeChoice &&
                null != this.activeChoice &&
                (this.quest_id == question_id || null == this.quest_id)
            );
        },
        isDisabledF(id) {
        ...
        cancelUpdate(id, index) {
            ...
            var tempOldData = oldQuestions.filter(
                data => data.question_id == id
            );
            var tempNewData = this.questions.filter(
                data => data.question_id == id
            );
            ...
            if (this.status === false) {
                ...
                if (
                    JSON.stringify(tempNewData) == JSON.stringify(tempOldData)
                ) {
                        ...
                        .then(confirm => {
                            if (confirm) {
                                ...
                                setTimeout(() => {
                                    this.addUpdateModel();
                                    this.$store
                                        .dispatch(constants.editQuestion)
                                        .then(() => {
                ...
                this.$refs.confirm
                    ...
                    .then(confirm => {
                        if (confirm) {
                            ...
                            this.$store
                                .dispatch(constants.deleteQuestion, id)
                                .then(() => {
                                    this.fullscreenLoading = false;
                                    if (
                                        !this.$store.getters[
                        ...
                        .then(confirm => {
                            if (confirm) {
                                ...
                                setTimeout(() => {
                                    this.addToModel();
                                    this.$store
                                        .dispatch(constants.addQuestion)
                                        .then(() => {
                                            var message = this.$store.getters[
                                                constants.listTemplate
                                            ].message;
        ...
        dispatchAll() {
            this.$store
                .dispatch(constants.templateViewDetails, this.$props.templateId)
                .then(() => {
                    setTimeout(() => {
                        this.loaded = true;
                        var message = this.$store.getters[
