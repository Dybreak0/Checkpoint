        ...
        <v-form>
            <v-layout row>
                <v-card flat class="search-filter-card">                    
                    <h2><v-icon>description</v-icon>TS JO Report</h2>
                    <v-spacer></v-spacer>
                </v-card>
            </v-layout>
            ...
            <v-flex xs12 m10>
                <v-data-table 
                    ...
                    <template v-slot:items="props">
                        ...
                        <td class="text-xs-left">{{ props.item.job_order_subject }}</td>
                        ...
                        <td class="text-xs-left">{{ props.item.status_name }}</td>
                    </template>
                </v-data-table>
...
<script>
    ...
    export default {
        name: constants.jobOrderReportPage,
        data() {
            return {
                pages: 0,     
                listLength: 0,      
                ...
                headers: [
                    ...
                    { text: constants.jobOrderSubject, align: constants.defaultTableAlign, sortable: false, width: "30%" },
                    { text: constants.reportedByHeader, align: constants.defaultTableAlign, sortable: false, width: "15%"},
                    { text: constants.jobOrderDateStartHeader, align: constants.defaultTableAlign, sortable: false, width: "8%"},
                    { text: constants.jobOrderDateEndHeader, align: constants.defaultTableAlign, sortable: false, width: "8%"},
                    { text: constants.applicationTypeHeader, align: constants.defaultTableAlign, sortable: false, width: "8%"},
                    { text: constants.statusHeader, align: constants.defaultTableAlign, sortable: false, width: "8%"}
                ],
                currentCaseNumber: "",
        ...
        methods: {
            initialize() {
                this.$store.dispatch(constants.jobOrderReportNew);           // Initialize all properties of the search model
                this.$store.dispatch(constants.jobOrderReportLoadOptions);   // Initialize and load all dropdown options
            },
            onPageChange(page) {
                this.newPage = page;
            ...
            clearFilters() {
                this.$store.dispatch(constants.jobOrderReportClearFilters);     // Resets the search filters to the default values
                this.search();
            },
            search() {
                this.fullscreenLoading = true;
                this.searchModel.page = (this.isPagination) ? this.newPage : 1;
                this.$store.dispatch(constants.jobOrderReportList).then(() => {   // Searches for job order records using the search filters 
                    ...
                    if (this.errorMessage === null) {                        
                        ...
                        this.currentStatus = this.searchModel.status;
            ...
            download() {
                ...
                this.searchModel.status = this.currentStatus;
                ...
                this.$store.dispatch(constants.jobOrderReportDownload).then(() => {     // Retrieves all the job order records and puts it into an excel file
                    this.fullscreenLoading = false;
                    if (this.errorMessage !== null) {
                        this.$refs.info.open(constants.warning, this.errorMessage, { color: constants.error_color });      
                    }
            ...
            view(id) {
                ...
                this.$router.push({ 
                    ...
                    params: { 
                        id: id.toString(), 
                        prevPage: constants.jobOrderReport 
                    }
                });
            },
        },
        computed: {
            ...mapGetters({
                list: constants.jobOrderReportList,
                options: constants.jobOrderReportOptions,
                searchModel: constants.jobOrderReportSearchModel,
